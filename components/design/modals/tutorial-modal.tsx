...
import type { OrgRole } from "@/types/design";
    ...
    <Dialog.Root open={open} onOpenChange={(v) => !v && closeModal()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-0 z-50 flex items-center justify-center p-4 focus:outline-none">
          <div className="relative w-full max-w-2xl rounded-2xl bg-surface border border-border shadow-[var(--shadow-card)] p-6">
            <Dialog.Close asChild>
              <button className="absolute top-4 right-4 w-8 h-8 rounded-full bg-surface-muted text-muted flex items-center justify-center hover:bg-border-strong transition-colors" aria-label="Close tutorial">
                <X size={16} />
              </button>
            </Dialog.Close>
              ...
              {ROLES.map(({ id, label }) => (
                <button key={id} onClick={() => toggleTutorialRole(id)}
                  className={cn("min-h-[64px] rounded-xl border px-4 py-4 text-left font-semibold text-sm transition-all",
                    tutorialRoles.includes(id) ? "border-accent bg-surface-muted text-text" : "border-border bg-surface-soft text-text hover:border-border-strong")}>
                  {label}
                </button>
              ))}
            </div>
