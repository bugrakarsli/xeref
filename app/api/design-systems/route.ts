...
export async function GET() {
  ...
  const { data, error } = await admin.from("design_systems").select("*").eq("org_id", orgId).order("created_at", { ascending: false });
...
export async function POST(req: NextRequest) {
  ...
  const admin = createAdminClient();
  const { data, error } = await admin.from("design_systems")
