...
# ── SQL migration ──────────────────────────────────────────────────────────────
sql = '''\
-- Xeref Design: Full Supabase schema
...
CREATE POLICY "users read own projects" ON projects FOR SELECT USING (owner_id=auth.uid());
CREATE POLICY "org members read org-visible" ON projects FOR SELECT
  USING (visibility=\'org\' AND org_id IN (SELECT org_id FROM org_members WHERE user_id=auth.uid()));
CREATE POLICY "users insert own projects" ON projects FOR INSERT WITH CHECK (owner_id=auth.uid());
CREATE POLICY "users update own projects" ON projects FOR UPDATE USING (owner_id=auth.uid());
CREATE POLICY "users delete own projects" ON projects FOR DELETE USING (owner_id=auth.uid());
...
# Write SQL migration
sql_path = os.path.join(BASE, "supabase/migrations/20260418_xeref_design.sql")
